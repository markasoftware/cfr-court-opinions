...
def title_xml(work_dir: WorkDir, year: int, month: int, title: int) -> None:
    ...
    def _iter_structure(structure, wanted_type: str) -> Iterator:
        if structure["type"] == wanted_type:
            yield structure
        else:
            for child in structure.get("children", []):
                yield from _iter_structure(child, wanted_type)
    ...
    for chapter_structure in _iter_structure(title_structure, "chapter"):
        assert chapter_structure["type"] == "chapter"
        chapter = chapter_structure["identifier"]

        for part_structure in _iter_structure(chapter_structure, "part"):
